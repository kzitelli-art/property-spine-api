...
--
--  Release 0 Step 2. Proof evaluations, evaluation→attachment links,
--  activation history and the legacy cutover inventory. Additive only:
--  no existing column changes type, loses a constraint, or is dropped.
--
--  ── THIS FILE WAS DERIVED, NOT AUTHORED ─────────────────────────────
--  Its DDL body is the exact bytes of the artifact measured by the 100k
--  row scale proof:
--
--      tools/scale/137_release_0_payload.sql
--      sha256 ae4b9b774cd9be8568ea24219d4ee4a98350b2bce3a03baf8de33d0cfcc2ea4d
--
--  It was produced by removing that file's own `begin;`/`commit;` and
--  replacing its header comment. Nothing else. Retyping it by hand would
--  have made the scale proof a statement about a different file.
--
--  tests/gate_migration_137_promotion.js compares the two statement by
--  statement on every run and fails if any substantive byte diverges.
--
--  ── WHY THERE IS NO begin/commit HERE ───────────────────────────────
--  migrations/migrate.js already issues `begin`, runs this file, INSERTS
--  THE schema_migrations ROW, and only then commits. A `commit;` in this
--  file would land the DDL before the ledger row, so a crash between them
--  would leave schema 137 applied with nothing recording that it was —
--  precisely the split-brain the ledger exists to prevent.
--
--  ── ROLLBACK IS BEHAVIOURAL, NOT DESTRUCTIVE ────────────────────────
--  Rolling Release 0 back means reverting the WRITER. This migration
--  stays. Every object it creates is new, so the previous API boots and
--  reads unchanged against it; dropping it would strand any evaluation
--  already written and is never the rollback path.
-- ════════════════════════════════════════════════════════════════════
