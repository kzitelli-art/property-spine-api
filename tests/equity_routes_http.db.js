/* ════════════════════════════════════════════════════════════════════
   ...
   2  the walls survive serialisation — E1–E10 are proven against
      position() elsewhere; this asserts they are still intact after
      the payload becomes JSON and crosses HTTP, reading keys BY NAME.
...
(async () => {
  try {
    ...
    const entity = await svc.establishCapitalEntity(c, {
      property_id: prop, entity_kind: "llc_membership", legal_entity_id: interestHolder,
      tier_order: 1, effective_from: "2020-07-31", source_artifact_id: art, recorded_by_user_id: uid });
    const position = await svc.addPosition(c, {
      capital_entity_id: entity.id, position_kind: "preferred_class", legal_entity_id: msc,
      effective_from: "2020-07-31", source_artifact_id: art, recorded_by_user_id: uid });
    await svc.addPreferredTerms(c, {
      position_id: position.id, term_source: "governing_document", source_authority: "governed_read",
      rate_bp: 1250, compounding: "quarterly", effective_from: "2020-07-31",
      source_artifact_id: art, recorded_by_user_id: uid });

    //  A second property with an equity entity secured by NOTHING to do
    //  with `prop` — the authority-bypass fixture, same shape as Debt's
    //  `foreign` instrument.
    const foreignEntity = await svc.establishCapitalEntity(c, {
      property_id: other, entity_kind: "llc_membership", legal_entity_id: interestHolder,
      tier_order: 1, effective_from: "2022-01-01", recorded_by_user_id: uid });
    await svc.addPosition(c, {
      capital_entity_id: foreignEntity.id, position_kind: "common",
      party_name_text: "OTHER-PROPERTY-HOLDER-999", effective_from: "2022-01-01",
    ...
    ok("...and MSC's real position does not leak into 4233's response",
       !/preferred_class/.test(mirror.raw) || !mirror.raw.includes(msc));
    ...
    ok("...and says NOT_ESTABLISHED rather than implying no equity exists",
       empty.json && empty.json.capital_entities.length === 0
       && empty.json.standing.truth_state === "NOT_ESTABLISHED");

    console.log("\n  ── E1–E10 survive serialisation (keys read BY NAME) ──");
    const st = await request({ path: "/operator/equity/standing", token: "entitled" });
    ok("200, real data", st.status === 200);
    const entities = st.json.capital_entities;
    ok("the capital entity is present", entities.length === 1 && entities[0].capital_entity_id === entity.id);
    const posOut = entities[0].positions[0];
    ok("MSC's position reads back preferred_class, by name",
       posOut.position_kind === "preferred_class");
    ok("...with its rate as-stated, quarterly, by source_authority — not a bare number",
       posOut.preferred_terms[0].source_authority === "governed_read"
       && posOut.preferred_terms[0].compounding === "quarterly");
    ok("E3 · accrued_preferred_return is NOT_ESTABLISHED over the wire, never a computed figure",
       posOut.accrued_preferred_return.truth_state === "NOT_ESTABLISHED");
    ok("E9 · no pledge recorded reads NOT_ESTABLISHED, never a false 'unencumbered'",
       posOut.encumbrance.truth_state === "NOT_ESTABLISHED");
