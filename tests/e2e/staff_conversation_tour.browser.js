...
(async()=>{
 ...
 try{
  ...
  const person=await one("insert into persons(name) values('Browser owned staff') returning id");
  const user=await one("insert into users(name,role,is_active,status,account_kind,person_id) values('Browser owned staff','leasing_agent',true,'active','human_staff',$1) returning id",[person.id]);
