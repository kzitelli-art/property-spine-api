...
(async () => {
  ...
  console.log(`PROOF_RUNTIME=${commit} mode=${parent ? "expected-legacy-open" : "retired"}`);
  ...
  const file = await one(`insert into deal_intake_files(intake_id,original_filename,detected_document_type,extracted_text,ingest_run_id)
    values($1,'original-rent-roll.csv','rent_roll','Unit,Beds,Rent\nProof A,1,1500',$2) returning id`, [intake.id, run.id]);
  ...
  if (parent) {
    ...
    for (const [method, path, options] of routes) {
      ...
      check(`${method} ${path}: existing anonymous gate remains 401`, anonymous.status === 401, anonymous);
