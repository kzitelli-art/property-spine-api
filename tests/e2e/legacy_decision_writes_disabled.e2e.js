...
async function plaidPost(rawBody, { verification = null, operatorKey = false } = {}) {
  ...
  const response = await fetch(BASE + "/plaid/webhook", {
...
(async () => {
  ...
  // ── PLAID PROVIDER AUTHORITY · raw bytes before any status note ──
  const plaidItemId = `item_${STAMP}`;
  await q(
  ...
  async function plaidDurableState() {
    const row = (await q(
  ...
  console.log("\n── PLAID VERIFIED RAW-BODY WEBHOOK ──");
  async function invalidPlaid(label, rawBody, verification, expectedStatus, operatorKey = false) {
    await resetPlaidItem();
    const beforeState = await plaidDurableState();
    const response = await plaidPost(rawBody, { verification, operatorKey });
    const afterState = await plaidDurableState();
    console.log(`  ${label} -> ${response.status} ${JSON.stringify(response.body)} state=${JSON.stringify(afterState)}`);
  ...
  const afterValid = await plaidDurableState();
  ...
  const afterDuplicate = await plaidDurableState();
  ...
  const afterUpdates = await plaidDurableState();
  ...
  const duplicateUpdates = await plaidPost(updatesRaw, { verification: plaidVerification(updatesRaw) });
  const afterDuplicateUpdates = await plaidDurableState();
  ...
  try {
    ...
    await q("delete from plaid_item where item_id = $1", [plaidItemId]);
  ...
  const plaidLeftover = await q(
    "select count(*)::int n from plaid_item where item_id=$1", [plaidItemId]);
  must("Plaid webhook harness fixture removed", plaidLeftover.rows[0].n === 0);
