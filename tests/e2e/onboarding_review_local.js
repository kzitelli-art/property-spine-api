...
(async () => {
  ...
  await startServer();
  const runCountBrowser = async phase => run(process.execPath, [path.join(appRoot,"canonical_onboarding_review.browser.js")], {
    cwd:appRoot, env:{...process.env,SP:ROOT,API:process.env.E2E_API_BASE,SESSION:session,
      PROOF_PHASE:phase,PROOF_ZERO_STATE:path.join(process.env.PROOF_OUTPUT_DIR,"zero-state.private.json"),
      PROOF_CLAIM_STATE:path.join(process.env.PROOF_OUTPUT_DIR,"uncorroborated-state.private.json"),
  ...
  if (!baselineMode && !spaceParentMode) {
    if (process.env.PROOF_ZERO_BROWSER === "1") {
      ...
      await runCountBrowser("zero");
    }
    if (process.env.PROOF_CLAIM_BROWSER === "1") {
      await run(process.execPath,[path.join(ROOT,"tests/proofs/availability_uncorroborated_claim.db.js")]);
      await runCountBrowser("claim");
    }
