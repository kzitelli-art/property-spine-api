...
//
//  Run:  HARNESS_DATABASE_URL="..." node tests/proofs/test_identity_bridge.db.js
// ════════════════════════════════════════════════════════════════════
"use strict";
...
const buildStaffBridge = require("../../src/identity/staff_bridge.js");
...
//
//  tests/proofs/test_release3.db.js already solved this with a per-run token; this is
...
(async () => {
  ...
  await tx(async (c) => {
    ...
    //  staff_bridge.js:55-63 records a DELIBERATE TIGHTENING: an admin session
  ...
  // ────────────────────────────────────────────────────────────────
  // THE CLOSURE AUTHORITY. leasing_conversion.js fails CLOSED without it, and
  ...
  await T("J1  gate PASSES on the patched tree", async () => {
    execFileSync(process.execPath, [path.join(__dirname, "..", "gates", "gate_no_raw_bridge_joins.js")], { stdio: "pipe" });
  });
  await T("J2  gate FAILS when a raw join is planted", async () => {
    ...
    let failed = false;
    try { execFileSync(process.execPath, [path.join(__dirname, "..", "gates", "gate_no_raw_bridge_joins.js")], { stdio: "pipe" }); }
    catch (_) { failed = true; }
    fs.unlinkSync(plant);
