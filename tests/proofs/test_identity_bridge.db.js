...
(async () => {
  ...
  await T("J1  gate PASSES on the patched tree", async () => {
    execSync(`node ${path.join(__dirname, "../gates/gate_no_raw_bridge_joins.js")}`, { stdio: "pipe" });
  });
  await T("J2  gate FAILS when a raw join is planted", async () => {
    ...
    let failed = false;
    try { execSync(`node ${path.join(__dirname, "../gates/gate_no_raw_bridge_joins.js")}`, { stdio: "pipe" }); }
    catch (_) { failed = true; }
    fs.unlinkSync(plant);
