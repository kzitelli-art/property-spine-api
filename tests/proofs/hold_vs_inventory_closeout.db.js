...
(async () => {
  ...
  try {
    ...
    // ── 3. THE DOWN HOLD AND THE TWO OCCUPANCY OWNERS ──────────────────
    {
      ...
      //  Compared against the RECORDED pre-hold (post-correction) values, not
      //  against itself: the earlier form of this assertion was a tautology.
      const preHold = evidence.sections.source_correction.consumers;
      ok("3: the Rent Roll buckets and standing tally do not move under down — placeholder bucket, Room1 bucket and standing open equal their recorded pre-hold values",
        c2.rent_roll.placeholder_bucket === preHold.rent_roll.placeholder_bucket && c2.rent_roll.room1_bucket === preHold.rent_roll.room1_bucket && c2.standing.open === preHold.standing.open && preHold.standing.open === 3, JSON.stringify([c2.rent_roll, preHold.rent_roll, c2.standing.open, preHold.standing.open]));
      ok("3: resolve restores EVERY consumer field and EVERY financial field to the recorded post-correction values (full payload comparison)", resolve.status === 200 && JSON.stringify([c3, f3]) === JSON.stringify([evidence.sections.source_correction.consumers, evidence.sections.source_correction.financial]), JSON.stringify([c3, f3]).slice(0, 800));
    }
    ...
    // ── 4. RETIREMENT + REINSTATEMENT, FULL IDENTITY COMPARISON ───────
    {
      ...
      //  On 139e696 the primitive counted 5 rentable while the unit was
      //  retired (no NOT_RETIRED predicate). QB's edceeb5 adds the predicate;
      //  this observation records whichever tree it runs on.
      ok("4 (note): while the unit was retired the occupancy primitive counted " + fRet.occupancy_primitive.rentable + " rentable — 2 means the retirement predicate is applied (edceeb5), 5 means it is not (139e696)", [2, 5].includes(fRet.occupancy_primitive.rentable), JSON.stringify(fRet.occupancy_primitive));
      ok("4 (edceeb5): the primitive applies the retirement predicate on this tree", fRet.occupancy_primitive.rentable === 2, JSON.stringify(fRet.occupancy_primitive));
    }
