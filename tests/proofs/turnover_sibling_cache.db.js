...
(async()=>{
  ...
  try {
    ...
    for(const scenario of ['live','lease_only','future_arrival','future_departure','last_bed','conflicted_end','concurrent']) {
    ...
    for(const label of ['Bed A','Bed B']) {
      ...
      if(label==='Bed B' && ['future_departure','last_bed','conflicted_end'].includes(scenario)) {
    ...
    if(scenario==='conflicted_end') {
      const laterStart=new Date(Date.now()-10*86400000).toISOString().slice(0,10);
      await one("insert into leases(property_id,space_id,start_date,end_date,lease_status) values($1,$2,$3,$4,'active') returning id",[property.id,beds[1].space.id,laterStart,end]);
    }
    const before=await spacePosition(pool,{property_id:property.id});
    const siblingHeld=['live','future_departure','concurrent'].includes(scenario);
    ...
    const moveOut=()=>fetch(process.env.E2E_API_BASE+`/units/${unit.id}/move-out`,{
    ...
    if(scenario==='concurrent') {
      const responses=await Promise.all([moveOut(),moveOut()]);
      assert.deepEqual(responses.map(r=>r.status).sort(),[201,409],'simultaneous move-outs commit once and refuse the competing turn');
      const refused=await responses.find(r=>r.status===409).json();
      assert.equal(refused.code||refused.error,'TURNOVER_ALREADY_ACTIVE','competing turn refusal has the canonical reason');
      response=responses.find(r=>r.status===201);
      assert.equal((await one('select count(*)::int n from turnovers where unit_id=$1',[unit.id])).n,1,'one durable turnover after the race');
    } else response=await moveOut();
    ...
    assert.equal(events.filter(e=>e.space_id===beds[1].space.id).length,['future_departure','last_bed','conflicted_end'].includes(scenario)?1:0,'no additional sibling possession event');
    ...
    if(scenario==='conflicted_end') assert.equal(after.positions.find(p=>p.space_id===beds[1].space.id).conflict_state,'conflicted','overlapping sibling rights are actually present');
