...
const PRODUCTION_APPROVED = [
  ...
  { file: "tools/release0/where_are_we.js",
    reason: "Release 0 activation instrument — must read PRODUCTION to answer which boundaries have actually landed, because a checklist records what somebody believes and this release has already paid twice for that (a PR reported merged whose later commits never landed; a deploy whose running SHA was not the one assumed). Every line is derived from the database or the running checkout. It opens a PROVEN READ-ONLY transaction before any read and cannot write; it does not deploy, migrate, activate or sweep. It emits ledger versions, table/index presence, counts and timestamps — never a phone number, media URL, credential or secret value (secret presence is computed in JS as a boolean, never a shell expansion)" },
  { file: "tools/step4/preflight.js",
    reason: "Release 0 Step 4 preflight — must read PRODUCTION to answer whether the §7.4 handset proof can be attempted at all: the ledger, the 137 objects, the single active operations line, the tester's resolution and active maintenance assignment, the activation state, and which work orders are SAFE to complete. It opens a PROVEN READ-ONLY transaction before any read and cannot write. It prints phone numbers as last4 only, and secret PRESENCE is computed in JS as a boolean — never a shell expansion of the value" },
  { file: "tools/step4/prove_completion.js",
    reason: "Release 0 Step 4 verification — the release's load-bearing gate (§7.4). Must read the exact PRODUCTION rows a real handset produced and check the eight completion facts, the no-parallel-writer facts and the operator surface. It opens a PROVEN READ-ONLY transaction before any read and cannot write; the completion itself is performed by the technician's message through the canonical service, never by this tool. Its assertions are proven and falsified in isolation first (tools/step4/rehearse.js). It emits ids, states, byte sizes and truncated digests — never a phone number, media URL, image bytes or message body" },
  { file: "tools/run_proof_defect_sweep.js",
    reason: "Release 0 §4.2 governed manual runner — the pass an operator types, NOT a scheduler, mirroring run_followups.js: DRY RUN by default and --raise must be typed. It is production-facing on purpose, and it decides nothing: every rule lives in the service, which itself REFUSES without a cutover activation and without migration 138. It writes only obligations, never work orders, and never closes anything" },
  { file: "tools/step7/census.js",
    reason: "Release 0 Step 7 cutover census — must read the PRODUCTION terminal-without-evaluation set, because §6.2 forbids using the August 6 audit as the deployment-time expected set and a fresh census is the whole point; proves it cannot write before any read; REFUSES to run without R0_CENSUS_AUTHORIZATION naming who authorized it, which is printed into the receipt; emits work-order and property identifiers, status and two booleans about the legacy columns — never a phone, a media URL, attachment bytes or note contents" },
  { file: "tools/steps23/verify_step3_deployed.js",
    reason: "Release 0 Step 3 running-checkout verification, OWED since the merge — must read the PRODUCTION checkout and ledger, because a green deploy event proves a build ran and not that the running bytes are the reviewed ones, and every boundary after Step 3 assumes the canonical writer is live. It opens a PROVEN READ-ONLY transaction before any read and cannot write; it compares file digests to the ones merged at b4e3104 and asserts the writer's ORDERING (evaluation before status). It emits digests, a ledger version and one count — never a phone, media URL, attachment bytes or note contents. NOTE: the behaviour classifier marks it write-capable because the word 'commit' appears in its PROSE; the classifier is deliberately blunt and is not being softened for this",
  },
