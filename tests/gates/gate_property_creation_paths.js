...
/* ════════════════════════════════════════════════════════════════════
   ...
   run:  node tests/gates/gate_property_creation_paths.js
   ════════════════════════════════════════════════════════════════════ */
"use strict";
...
const CALLERS = [
  ...
  {
    //  Moved verbatim out of server.js — the file field tracks the door,
    //  not the old address. Same route, same source_tag, same rules.
    file: "src/baseline/baseline_routes.js",
  ...
  {
    file: "src/onboarding/deal_intake.js",
...
ok("B2  bank_intake's CANONICAL-KEY IDEMPOTENCY survived the collapse",
   /idempotent/.test(writerSrc) && /created:\s*false/.test(writerSrc),
   "the same canonical key can now produce a duplicate property");

ok("B3  deal_intake's TEACH-ON-CREATION survived the collapse",
...
const PROOFS = ["tests/proofs/property_creation_canonical.db.js", "tests/proofs/property_creation_http.db.js"];
