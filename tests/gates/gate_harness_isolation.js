...
/* ════════════════════════════════════════════════════════════════════
   ...
   USAGE / EXIT
     node tests/gates/gate_harness_isolation.js
...
const PRODUCTION_APPROVED = [
  ...
  { file: "tests/scenarios/smoke_release2.deployed.js",
    reason: "deployment smoke — targets the DEPLOYED service by design" },
  { file: "tests/scenarios/smoke_release3.deployed.js",
    reason: "deployment smoke — targets the DEPLOYED service by design" },
  ...
  { file: "tools/debt/establish_instrument.js",
    reason: "The controlled Debt establishment step — WRITE-CAPABLE BY DESIGN, and broader " +
            ...
            "human confirms them through the product. Proven by tests/proofs/debt_establishment_tool.db.js, " +
  ...
  { file: "tools/turn_readiness_census.js",
    ...
    until: "`turnovers.ready_date` is retired (docs/archive/TURN_READINESS_SEMANTICS_TRACE.md §4)" },
  { file: "tools/leasing_basis_discovery.js",
    reason: "Leasing cycle basis discovery — must read the database holding the FRESH " +
            ...
            "produced a wrong answer (docs/archive/LEASING_CYCLE_AND_PACE_TRACE.md §4). READ-ONLY: " +
  ...
  { file: "tools/repair/resolve_surplus_whole_unit_spaces.js",
    reason: "PLATFORM REPAIR, and repair authority is narrower than management authority. " +
            ...
            "removes a unit's last position. Proven in tests/proofs/surplus_placeholder_repair.db.js " +
...
const FROZEN_INVENTORY = [
  { path: "tests/proofs/agent_booking_xturn_proof.js",
  ...
  { path: "tests/unit/application_submission.test.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/billback_decision_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/unit/birth_guard.test.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/unit/capability_contract.test.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/capture_chase_proof.js",
    write: false, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/comms_boundary_phase_a_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/consent_and_scope_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/demo_book_route_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/scenarios/demo_enrollment_dry_run.js",
    write: false, use: "analysis",
    reason: "pre-existing analysis/backfill tool at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/unit/executed_lease_overlap_concurrency.test.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/fact_write_resilience_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/arcs/full_lifecycle_arc.js",
    write: false, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/inbound_prospect_resolution_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/intake_source_fallback_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/lead_source_attribution_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/unit/lease_void_contract.test.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/legacy_agent_routes_removed_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/arcs/movein_arc.js",
    write: false, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/arcs/movein_beat.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/never_delete_guard_proof.js",
    write: false, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/scenarios/night_harness.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/unit/owner_eligibility_contract.test.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/scenarios/ownership_reachability_check.js",
    write: false, use: "unknown",
    reason: "pre-existing at the measured SHA; use not yet determined by reading it",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/person_facts_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/presence_wall_lease_proof.js",
    write: false, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/scenarios/proof_proposed_terms.js",
    write: true, use: "unknown",
    reason: "pre-existing at the measured SHA; use not yet determined by reading it",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/property_capability_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/scenarios/prove_escalate_move.js",
    write: true, use: "unknown",
    reason: "pre-existing at the measured SHA; use not yet determined by reading it",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/scenarios/prove_one_in_one_out.js",
    write: true, use: "unknown",
    reason: "pre-existing at the measured SHA; use not yet determined by reading it",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/scenarios/prove_persona_v6.js",
    write: true, use: "unknown",
    reason: "pre-existing at the measured SHA; use not yet determined by reading it",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/arcs/qa_lifecycle_arc.js",
    write: false, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/readiness_certification_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/relationship_stage_proof.js",
    write: false, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/renewals_slice6_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/unit/send_action_basis_contract.test.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/unit/shadow_import.test.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/slice8_governed_economics_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/slice9_appointment_journey_proof.js",
    write: false, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/slice9_attribution_backfill_proof.js",
    write: true, use: "analysis",
    reason: "pre-existing analysis/backfill tool at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/slice9_attribution_writer_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/slice9_evidence_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/slice9_inbound_decision_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/slice9_lifecycle_authority_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/slice9_lifecycle_concurrency_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/slice9_metric_contract_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/slice9_operating_timezone_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/slice9_opportunity_funnel_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/slice9_opportunity_lifecycle_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/slice9_path_a_birth_cutover_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/slice9_paths_bcde_cutover_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/slice9_status_read_correction_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/slice9_timezone_command_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/scenarios/smoke_proposed_terms_route.js",
    write: true, use: "unknown",
    reason: "pre-existing at the measured SHA; use not yet determined by reading it",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/staff_agent_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/standing_context_proof.js",
    write: false, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/tour_booking_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/tour_chips_proof.js",
    write: false, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/tour_outcome_capture_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/unit/tour_scheduled_for.test.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/unit/tours_conveyor.test.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/unit_triage_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/unit_turn_scope_proof.js",
    write: false, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/walk_in_tour_proof.js",
    write: true, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/work_acceptance_proof.js",
    write: false, use: "proof",
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "converted to harnessConnectionString() and executed against an isolated database, or deleted as dead, in the remediation slice" },
  { path: "tests/proofs/work_order_authority_proof.js",
    write: true, use: "proof",
    sliceABlocker: true,
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "repaired and proven BEFORE Slice A merges — directly and through the suite runner (owner ruling 2026-08-03)" },
  { path: "tests/proofs/work_order_canonical_path_proof.js",
    write: true, use: "proof",
    sliceABlocker: true,
    reason: "pre-existing proof harness at the measured SHA; not yet triaged",
    until: "repaired and proven BEFORE Slice A merges — directly and through the suite runner (owner ruling 2026-08-03)" },
...
const FROZEN_HARNESS_VAR_DIRECT = [
  { path: "tests/proofs/hotfix_future_rent_roll_guards_proof.js", write: true,
  ...
  { path: "tests/proofs/slice9_evidence_http_proof.js", write: true,
    reason: "pre-existing Slice 9 harness; requires the harness var, performs no same-target refusal",
    until: "converted to harnessConnectionString() and executed against an isolated database" },
  { path: "tests/proofs/slice9_inbound_decision_http_proof.js", write: true,
    reason: "pre-existing Slice 9 harness; requires the harness var, performs no same-target refusal",
    until: "converted to harnessConnectionString() and executed against an isolated database" },
  { path: "tests/proofs/slice9_scale_proof.js", write: true,
    reason: "pre-existing Slice 9 harness; requires the harness var, performs no same-target refusal",
    until: "converted to harnessConnectionString() and executed against an isolated database" },
