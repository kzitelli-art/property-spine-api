...
//      node tests/scenarios/smoke_release3.deployed.js
//
...
(async () => {
  ...
  try {
    ...
    const bridgeSvc = require("../../src/identity/staff_bridge.js")({ pool })._service;
    ...
    if (OPKEY) {
      ...
      ok(r.status === 404, "2  the legacy completion door is RETIRED (404) — completion invariants moved to tests/proofs/obligation_completion_canonical_proof.db.js", r.status);
    }
    ...
    if (OPKEY) {
      const r = await call("PATCH", `/obligations/RETIRED/complete`, { key: OPKEY, body: {} });
      const b = await j(r);
      ok(r.status === 404, "the legacy completion door is RETIRED (404) — the conversion-rail refusal is proven in tests/proofs/obligation_completion_canonical_proof.db.js", r.status);
    }
...
//  tests/proofs/obligation_completion_canonical_proof.db.js, against the
...
//    BASE=https://…  OPERATOR_KEY=…  STAFF_SESSION=…  node tests/scenarios/smoke_release3.deployed.js
// ════════════════════════════════════════════════════════════════════
