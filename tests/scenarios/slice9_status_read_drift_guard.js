/* ══════════════════════════════════════════════════════════════════════════
   ...
   does NOT police availability.js, turn_priority.js or lease_packets.js —
   those are Pass 2, where the governing question is which fact actually holds
   a unit, not which statuses look progressed.

   Run: node tests/scenarios/slice9_status_read_drift_guard.js
   ══════════════════════════════════════════════════════════════════════════ */
"use strict";
const fs = require("fs");
...
const GUARDED = [
  ...
  "src/leasing/leasing_conversion.js",
  "src/applications/application_submission.js",
];
...
// lease_packets.js in Commit H, so no file remains in the "known-defective,
...
ok("lease_packets.js NO LONGER carries a private status list (Commit H)",
  !/\["lease_ready", "tenant_signed", "approved"\]/
    .test(code("src/applications/lease_packets.js")));
ok("and asks the named eligibility predicate instead",
  /assessLeasePacketEligibility/.test(code("src/applications/lease_packets.js")));
...
for (const f of ["src/maintenance/turn_priority.js", "src/applications/lease_packets.js",
...
const sub = code("src/applications/application_submission.js");
