...
//
//    node tests/scenarios/smoke_release2.deployed.js
//
...
(async () => {
  ...
  try {
    ...
    if (OPKEY) {
      ...
      const b1 = await j(r1);
      ok(r1.status === 404, "1  the legacy completion door is RETIRED (404) — completion invariants moved to tests/proofs/obligation_completion_canonical_proof.db.js", r1.status);
    }
    ...
    if (OPKEY) {
      ...
      const b2 = await j(r2);
      ok(r2.status === 404, "the legacy completion door is RETIRED (404) — the conversion-rail refusal is proven in tests/proofs/obligation_completion_canonical_proof.db.js", r2.status);
    }
