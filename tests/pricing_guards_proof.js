...
//
//  Run:  DATABASE_URL=... node tests/pricing_guards_proof.js
// ════════════════════════════════════════════════════════════════════
...
const head = (rel) => execSync(`git show HEAD:${rel}`, { cwd: REPO, encoding: "utf8", maxBuffer: 20e6 });
...
(async () => {
  ...
  ok(!/bedrooms is not null/.test(invHead), "HEAD (pre-fix): guard absent — so this assertion CAN fail");
  ...
  if (!url) {
    ...
    ok(!/lease_status not in/.test(invHead), "HEAD (pre-fix): committed-space guard absent — assertion CAN fail");
    ...
    ok(!/effective_until is null or effective_until > now\(\)/.test(agentHead),
      "HEAD (pre-fix): expiry ignored — assertion CAN fail");
  ...
  ok(headRung3.length > 0, "extracted pre-fix rung 3 text from git HEAD");
  ok(ECONOMICS.test(headRung3),
    `HEAD (pre-fix) rung 3 FAILS the economics assertion — proving the test can fail: "${headRung3.replace("${who}", "Casey").slice(0, 78)}…"`);
  ok(/2027/.test(headRung3), "HEAD rung 3 contained a hardcoded expiry year it could not enforce");
