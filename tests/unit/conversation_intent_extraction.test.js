...
//  answer recognition are extracted out of src/comms/tenant_link.js, where
...
//  diff: the two function bodies are byte-identical to the tenant_link
...
//
//  One implementation (tenant_link must delegate, never keep a copy), and
...
const TENANTLINK_SRC = path.join(__dirname, "..", "..", "src", "comms", "tenant_link.js");
...
try {
  ...
  section("EXTRACTION IDENTITY");
  {
    ...
    let prior = null;
    try {
      //  1454330 predates the tenantlink.js → tenant_link.js rename, so the
    ...
    if (prior) {
      ...
      //  tenant_link source, de-indented. That proves nothing was rewritten
      ...
      ok(`every extracted line (${substantive.length}) appears verbatim in the tenant_link original`,
        foreign.length === 0,
        foreign.length ? "changed lines: " + foreign.slice(0, 3).join(" | ") : null);
    } else {
      unproven("every extracted line appears verbatim in the tenant_link original",
  ...
  section("ONE IMPLEMENTATION");
  {
    ...
    ok("tenant_link keeps NO local classifyMessage", !/async function classifyMessage/.test(tl));
    ok("tenant_link keeps NO local recognizeAnswer", !/async function recognizeAnswer/.test(tl));
    ok("tenant_link keeps NO local classification vocabulary", !/const FIELD_CATEGORIES/.test(tl));
    ok("tenant_link keeps NO local emergency/sensitive patterns",
      !/const EMERGENCY_RX/.test(tl) && !/const SENSITIVE_RX/.test(tl));
    ok("tenant_link DELEGATES to the shared module", /require\("\.\.\/conversation\/intent"\)/.test(tl));
  }
