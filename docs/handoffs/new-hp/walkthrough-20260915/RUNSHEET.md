...
- Two-step Execute in the **browser** on the combined app `1a5f257`: the shipped proof timed out at the new "Choose a property" layer (never run in CI; skipped unless an app checkout is named). Adapted on `claude/two-step-browser-picker-20260914` (`140cf35`, proof only) to make the visible selection; rerun on a fresh owned database at ledger 198 with app `1a5f257`: **`two-step execute (browser): 22 checks passed`**, no page errors. Screenshots in `two-step-execute-browser/`: `choose_property_Mike.png`, `choose_property_KZ.png` (the picker, chosen visibly by each actor), `execute_mike_before.png`, `execute_mike_refused.png` (Mike cannot execute), `execute_kz_before.png`, `execute_kz_after.png` ("Lease executed and tenancy created — pending lease …", Application Review reads TENANT FILE OPEN, executed lease, signatures, move-in pending). Log: `evidence/two_step_execute_browser_140cf35.log`.
