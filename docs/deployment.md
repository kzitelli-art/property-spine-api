...
TWILIO_AUTH_TOKEN=...
<!-- TWILIO_FROM_NUMBER was documented here as a settable variable and was read by NO code (zero occurrences in src/ or server.js, 2026-09-16). The outbound `from` is resolved per property by communication_lines.resolveOutboundLine; a global default would be exactly the fallback that module refuses — a message appearing to come from a building it did not come from. Removed rather than wired. -->
