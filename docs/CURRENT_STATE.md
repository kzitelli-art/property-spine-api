...
claim for the position its confirmation resolved (`import_source_rows`
lineage), never for a unit-number text match or a count of one; a claim
...
at `7c0fc48`. **QB's September 7 return (relay findings 1–3):** reproduced
on the real readers in an owned database and falsified against a parked
patch — `docs/handoffs/desktop-qb-20260906/availability-basis/fable-reset2/`
(`relay.patch`, proof, witness and successor artifacts). Not committed to
`src/` or `tests/`: integration ownership is QB's. See the receipt's final
section.
...
| # | Defect | Evidence |
|---|---|---|
| 66 | **EVIDENCE ONLY, 2026-09-07 — two identity exposures reproduced through the writers, readiness gates traced to a matrix; nothing repaired.** (a) Present-null lineage cannot separate a valid sole-position confirmation from an ambiguous unit-level promotion: ingest links `produced_unit_id` for every bare row and `produced_space_id` for none on a bed property, so the reader separates them only by today's position count (row 63's ruling, unchanged; "exclusively old-writer" holds for the writers, not the table). (b) After `reinstateInventoryUnit` a unit-linked claim follows its durable unit; a null-lineage claim crosses onto a replacement carrying the number. The writer checks actor, reason and a live retirement, never whether live inventory carries `original_unit_number`; retirement does not rename, so the shape needs a manual rename past `uq_unit_per_property`. Proposed, not built: a named refusal in the writer. (c) A NAMED `(whole unit)` claim, or a `produced_space_id` link to a retained placeholder beside real beds, attaches and reads `marketable_now` (seven positions for five beds); row 64's bare-key refusal does not cover name or link. Owner `position_kind`; proposed read-side shape-conflict hold in QB's readers. (d) Readiness gates: marketing and application targeting move together (`evaluateOfferability` admits `marketable_now` only, exercised on the seven shapes), touring has no unit-state gate, delivery reads the rent-ready approval obligation, not certification. Three rulings left with Kameron and QB in the receipt. Rung: `Locally exercised` (owned Postgres, real modules, no HTTP); the identity proof is deliberately outside `verify_all.sh` because it pins the defects it argues for. | `tests/proofs/identity_lineage_challenge.db.js` (20/20, not in CI); `tests/proofs/availability_readiness_axis.db.js` (+application column, 15/15 both modes, in CI); `docs/handoffs/desktop-qb-20260906/identity-readiness/RECEIPT.md` + two scrubbed JSON artifacts |
...
| 63 | **RESOLVED ON CANDIDATE, 2026-09-06 — a historical opening claim acquired a new identity through the canonical space reader.** The candidate subquery in `src/tenancy/space_position.js` matched `proposed_records.natural_key` against today's `units.unit_number` text, and a bare unit key against any unit with exactly one space today. Reproduced in an owned database on the unrepaired reader (`tests/proofs/opening_claim_identity.db.js`, `PROOF_EXPECT_DEFECT=1`, 17/17): a claim confirmed against a unit later retired for superseded grain re-attached to the replacement unit carrying the same number, with null lineage and with `import_source_rows.produced_space_id` pointing at the retired space alike; a linked claim was lost when its room was relabelled; a bare-unit claim attached to whichever bed a 3-bed unit had left; and on the replaced unit the Rent Roll unit view, canonical availability and Ask Spine all read open/marketable. Fix, reader only: `produced_space_id` → that position; `produced_unit_id` without a space → inside that unit, a named key by label and a bare key only when that unit has exactly one position, whatever its label (the ruling `canonical_onboarding_ledger.db.js` froze: a unit-key current claim answers for a unit's sole bed and an exact-key future row never outranks it), never one bed among several; no lineage → text, the unit's sole position for a bare key, and never once `inventory_retirements.original_unit_number` has carried the number. No timestamps. Text and the sole-position count are still used: text only where no lineage exists and never across a retirement, and the count only to decide which of a unit's positions a unit-level claim describes, never which unit. Successor 17/17 (linked whole-unit, legacy whole-unit and bed-by-bed confirmations still resolve; two-baseline date selection and operative-lease priority unchanged; all three readers agree; no proposal row rewritten); the witness re-run on the fix fails at exactly the four identity-drift assertions (case D, three beds shrunk to one, is a recorded control in both modes: it attaches under the sole-position ruling, a compatibility boundary, not a repaired drift). Space-availability successor, historical successor challenge and the five leasing suites PASS over real HTTP on the fixed reader. **Changes for existing data:** none for a unit that always had one position; a bare-unit claim on a unit whose bed set later shrank to one is indistinguishable from that without an inventory history and still attaches (recorded; no product writer deletes a space). **Recorded, not changed:** the legacy `/operator/rent-roll` projection (`snapshot_loader.js` `sourceRowPosition`) still resolves null-lineage rows by text with `matches.length === 1`. Rung: `PROVEN` (real Postgres) locally on Linux; in `verify_all.sh`; no CI run read at the time of writing. | `src/tenancy/space_position.js`; `tests/proofs/opening_claim_identity.db.js`; `tests/e2e/verify_all.sh`; `docs/handoffs/desktop-qb-20260906/availability-basis/RECEIPT.md` |
