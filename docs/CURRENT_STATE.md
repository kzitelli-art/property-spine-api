...
| 30 | **`tools/seed_solo_facts.js --confirm` CANNOT WRITE.** `virtual_tours` is in the `FACTS` array (`:214`) but absent from `CATEGORY_FOR` (`:73-83`), so the insert passes `undefined` into `agent_facts.category`, which is `not null` — the whole transaction rolls back. Dry run returns before the insert, so it looks healthy. Found by source scrub, not by running it (no DB in session), so this is REPORTED, not reproduced. Also dead in the other direction: `communication_instructions` is mapped at `:75` and has no fact. | `tools/seed_solo_facts.js:73-83,214`; `migrations/053_agent_supervised_drafts.sql:116` |
| 31 | **There are TWO Solo fact corpora with near-disjoint vocabularies, and the uniqueness constraint cannot see the conflict.** `tools/seed_solo_facts.js` (18 keys, 12 categories) and `tools/data/demo_solo_agent_facts_v1.json` (19 rows, 7 categories) overlap on exactly two keys — `pet_policy`, `renters_insurance`. The rest are synonym pairs (`utilities`/`utilities_policy`, `parking_pricing`/`parking_rules`, `amenities`/`amenities_list`, `move_in_requirements`/`move_in_process`, `screening_process`/`required_documents`, `sms_contact_line`/`office_contact`), plus one blob vs five itemized fee keys. `uq_agent_facts_property_active` is on `(property_id, fact_key)`, so it protects the NAME, not the fact: two synonym keys can both be active with different content and `resolveContext` reads BOTH into VERIFIED PROPERTY FACTS. 27 distinct `fact_key` literals exist across the tree. Standardized model proposed in `docs/PROPERTY_FACT_BASELINE_MODEL.md`. | `tools/seed_solo_facts.js`; `tools/data/demo_solo_agent_facts_v1.json`; `migrations/053_agent_supervised_drafts.sql:129-130`; `src/agent/agent.js:294-314` |
| 32 | **The one curated fact that must expire has no expiry, and curated facts have no activation boundary at all.** `src/agent/agent.js:296-303` honors `effective_until` and its own comment says "No live fact sets effective_until today" — but `current_concession`'s text is *"Concessions change month to month"* (`seed_solo_facts.js:206`) and the seed inserts it unset, so a July 2026 concession stays quotable indefinitely. Separately, `agent_facts` has only `status in ('active','retired')`: a fact is quotable the instant it is inserted. Migration `106_quote_activation_boundary.sql` already settled this for `property_governed_charges` (`record_state` + `quote_state`, receipted activation, one live owner per code per property); `053` predates it and never gained the second axis. | `src/agent/agent.js:296-303`; `tools/seed_solo_facts.js:206`; `migrations/106_quote_activation_boundary.sql`; `migrations/053_agent_supervised_drafts.sql:122` |
| 33 | **The universal leasing-agent prompt carries a titled, property-specific fact block.** `src/agent/agent.js:804-812` is `APPROVED SOLO PROFILE (stable building facts you may use directly)` — Solo's street address and neighborhood, its five layouts, furnished options, *"Apartments include in-unit laundry and kitchen appliances"*, its amenity list, and fitness-vs-golf-simulator hours. It is referenced by name from the rules at `:551`, and `:811` sits one line above *"FEES HAVE EXACTLY ONE APPROVED SOURCE: VERIFIED PROPERTY FACTS below"*. A second property inherits all of it. Only `:812` (assistance animals are not pets) belongs there, being federal law rather than a property fact; `:805-811` are all `fact_key`s that already exist, and `amenities` is now asserted in THREE places — the prompt, `amenities_list` in the seed, and `amenities` in the JSON corpus. Separately `:671` asserts Solo's past move-in speed. | `src/agent/agent.js:551,671,804-812` |
| 34 | **Corpus B curates availability, which `053` explicitly forbids, and it is months stale.** `tools/data/demo_solo_agent_facts_v1.json` carries `availability_as_of`: *"Unit availability and pricing reflect the property's May 31, 2026 operating report."* `053_agent_supervised_drafts.sql:106-108` states the rule it breaks — *"NOT a shadow rent roll: availability / rent / unit type are resolved LIVE from units."* No `effective_until` bounds it. Same corpus also carries `lease_terms` with a `15-20% premium` range, one of the seven range rows behind the dash-strip incident at `src/agent/agent.js:126-131`. | `tools/data/demo_solo_agent_facts_v1.json`; `migrations/053_agent_supervised_drafts.sql:106-108`; `src/agent/agent.js:126-131` |
| 35 | **PROPERTY ONBOARDING, found by the 16 Sep 2026 two-property leasing knowledge review — source documents are resolved by FILENAME, and one is mislabeled.** A file named as one property's lease template is actually a different property's lease, for another address and term; filename-based resolution would have used it. The review pins the other property's governing lease by SHA256 and rules that a newer file with different fee fields must not supersede it by filename or date alone. `work_order_proof_attachments.sha256` (`migrations/134_technician_lifecycle.sql`) already proves the digest primitive exists in this tree; leasing source records do not use it. Also found: a layout page whose embedded video points at an unrelated title and must not be used as a unit tour — a media asset whose reach and layout mapping were never established. Design response in `docs/PROPERTY_FACT_BASELINE_MODEL.md` §3.3 and §4.3. | 16 Sep 2026 leasing knowledge review packet; `migrations/134_technician_lifecycle.sql` |
