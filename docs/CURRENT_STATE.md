...
| 33 | **The universal leasing-agent prompt names Solo twice.** `src/agent/agent.js:671` ("Solo has sometimes moved people in within a few days...") and `:811` ("Solo is pet friendly, but current restrictions and charges must come from VERIFIED PROPERTY FACTS below") — the second sits one line above the rule that facts are the only approved source. A second property inherits both as prompt text. | `src/agent/agent.js:671,811` |
