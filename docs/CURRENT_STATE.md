...
| 34 | **W1 REPORTS THE EARLIEST PAYOFF STATEMENT, NOT THE LATEST — and every fixture in the repo conceals it.** `debt_position_read.js` establishes payoff with `balances.find((b) => b.observation_source === "payoff_statement" && onOrBefore(...))`. `loadHistory()` returns balance observations `order by as_of_date` **ascending**, and `.find` takes the first match — so with two payoff statements on one instrument the read reports the **older** quote. A payoff statement is a quote good through a date; the earliest is the one least likely to be actionable, and the amount is presented as established truth (W1's whole point is that payoff is its own key and is never aliased). Every existing fixture seeds exactly ONE payoff statement, so the earliest and the latest coincide and no test can go red. Reproduced by seeding two (2022-05-01 and 2025-05-01) in `tests/debt_observation_bound_equivalence.db.js`, where reading at 2025-05-02 returns the 2022 row. **This is a correctness defect, not a cost one** — found while bounding the same statement, deliberately kept out of the ceiling work. Not fixed: the right reading (latest? or a conflict when two are live?) is a product ruling, and it touches the same array as the tie in #35. | `src/asset/debt_position_read.js` payoff block; `tests/debt_observation_bound_equivalence.db.js` seeds the two-statement case |
| 35 | **TWO BALANCE OBSERVATIONS ON ONE DATE PRODUCE AN ARBITRARY WINNER, SILENTLY.** `position()` sorts balance observations with a comparator returning `-1` for equal keys, so among rows sharing an `as_of_date` the winner is input order — and `order by as_of_date` does not order equal dates. Nothing in the query or the reader decides it. Two balances recorded for one date from different sources is a **conflict**, and §5 says surface it rather than pick one. **RULED (2026-08-24): it surfaces as a conflict, not a silent winner.** Writer-side change, queued for reconciliation, deliberately not built here. This also **blocks the cost bound** on `debt_balance_observations`: the bound is proved correct at every date except a tie, where it merely changes which arbitrary row wins. `tests/debt_observation_bound_equivalence.db.js` holds it open with an assertion that goes RED once a conflict key appears, saying the bound can then be re-run. | `src/asset/debt_position_read.js` observed-balance sort; the tie section of `tests/debt_observation_bound_equivalence.db.js` |
