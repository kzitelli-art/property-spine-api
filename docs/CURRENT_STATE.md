...
APP verified/deployed   65dd76f     2026-08-21
...
| 19 | **RESOLVED AND DEPLOYED FROM THE INTEGRATION BRANCH, 2026-08-21 — not yet observed through a production staff SMS turn.** The operations SMS line can answer clear, supported read questions through the existing Ask Spine service. Recognized technician actions and work questions remain on the technician path; identity and property scope remain server-derived. Tour-schedule questions now use that same path on the release candidate. No new conversational pipe. Pure routing 30/30; real Express webhook + real PostgreSQL 77/77. | `src/conversation/staff_sms_router.js`; `src/comms/staff_governed_read.js`; `src/comms/staff_thread.js`; `src/comms/tenantlink.js`; deployed base `4f555c1` |
