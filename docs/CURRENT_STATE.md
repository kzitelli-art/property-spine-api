...
## Meeting Evidence and tenancy — `src/{meeting_evidence,tenancy}/`, migrations 175–188
...
| Meeting Evidence — mixed-property passage scope | **`HTTP_PROVEN`**, active branch only | Migration 188 and `src/meeting_evidence/meeting_passage_scope_service.js`. `tests/meeting_passage_scope_http.db.js` opened a real staff session through the real Express router against real Postgres (35/35): a four-property Temple specimen was classified mixed, whole-meeting binding was refused at HTTP and database layers, explicit partial/complete review coverage was retained, non-contiguous Skyline character ranges remained visible, Berks/Greenery/Temple Nest text and external-scope metadata remained absent from the property preview, a correction removed stale visibility, and canonical meetings/model attempts/receipts stayed at zero. `tests/meeting_passage_scope.test.js` adds 30/30 DB-free boundary/addressing/route refusals, including Unicode offset agreement. **Not merged, deployed, browser-observed, or production-observed.** This slice deliberately does not feed extraction or Ask Spine; that is the next governed connection after a real Temple payload is scoped. |
