...
| 22 | **FIRST RED IN THE LIVE SKYLINE TOUR WALK, 2026-08-21.** A controlled prospect asked to tour unit `1417-102`; the agent offered no slot and instead asked for move-in/move-out dates to pull pricing. No SMS was sent. The first gate is configuration: Render's `AGENT_TOUR_BOOKING_PROPERTY_IDS` contains only Demo, not Skyline. Behind it, Skyline also has `operating_timezone = null` and zero `tour_availability` rows, so enabling the flag alone would still offer nothing. Stop here before application/lease: establish timezone and real slots, then repeat this same turn. | Production draft `7a00aff5-3ec2-48de-b6e0-8dfefb1680f1`; Render env observed directly; production read of `properties` + `tour_availability` |
