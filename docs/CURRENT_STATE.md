...
**September 12 current candidate, not released:** explicit authenticated `attempt_sms:false` now captures the inquiry without generating a model response or inserting an unsent outbound text. Omitted/true attempts and the public demo retain their existing contracts; historical replay retains its original receipt. Owned focused HTTP26, unchanged inquiry-state42 and all56 source gates passed before integration. The canonical runner now requires the focused proof. Older delivery tests now expect no draft for capture-only and still challenge post-capture response failure plus real worker death in both capture-only and response-requested modes. New exact-SHA CI is pending. External initial-inquiry email recording remains a separate active writer/read/UI lane; application invitation attestation does not answer that gap. See [capture-only correction](handoffs/new-hp/website-reply/CAPTURE_ONLY.md).
