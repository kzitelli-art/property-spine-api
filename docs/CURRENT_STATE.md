...
**September 13 combined migration 194→198 release/recovery candidate — local only:** a pinned, tracked-clean wrapper now verifies exact physical and ledger states and delegates only the accepted suffix to the unchanged canonical per-file migration runner. The owned PostgreSQL 17 recovery matrix passed 52/52: clean apply, actual held-lock failure of each file, exact partial rollback state, explicit forward resume, a full 194 apply failing only at 198, restart/health twice, and same-name malformed catalog refusals. Focused source-home HTTP54/54, inventory policy63/63 with24 migration-197 trigger tables, contract23/23 and source governance56/56 passed. The matrix is separate from full CI because it requires its own exact-194 database; the pure contract is registered. Production remained read-only at PostgreSQL18.6 and exact194, so a separate owned18.6 catalog rerun remains required. Recovery after any committed suffix is forward-only with an API carrying the reached ledger and remaining migrations; no downgrade, deployment, provider, source-confirmation or property acceptance is claimed. [Receipt](handoffs/new-hp/combined-194-198-release/RECEIPT.md).
