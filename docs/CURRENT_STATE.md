...
| 66 | **LOCALLY EXERCISED · LAUNCH BLOCKERS NAMED · OWNER DECISIONS OWED, 2026-09-11 — The Greenery cannot be launched from the current source without two writes that have no door, and its first staff assignment depends on the shape of Mike's account.** A source trace and an owned-database rehearsal on `932c9af` reproduced the production read's shape for the existing Greenery property (171 units and placeholder spaces, 3 aliases, 1 deal-less activation, no organization, no team, basis unknown) and walked the smallest governed setup sequence: super-admin adoption → admin invite (existing account matched by exact email, platform role passed explicitly) → governed teammate invite with OTP → org-admin deal + add existing property → retained bed-grain rent roll → read-source with `leasing_basis: bed` → per-row confirmation → establish → property line → lease configuration with company signer → allowlisted intake and application birth → nine prepared knowledge cards → Ask Spine answer. 51 checks passed, 0 failed, 20 observations. Reproduced findings: (1) `deal_service.addProperty` admits an unowned property into an org's deal before adoption; (2) `/admin/organizations/:id/invite` and `/org/users/invite` 500 for phone-only accounts (`ON CONFLICT (phone)` has no matching constraint; `users` has only the normalized partial index) and 500 when the email differs from the existing account; (3) the admin door resets `platform_role` when omitted and writes no person-level `assignments` row, leaving the invitee's staff identity `not_assigned_here`; (4) beds materialised by Deal Setup carry no `spaces.use_type` and read `use_not_configured` until the reviewed `tools/apply_unit_type_mapping.js` run, which needs source rows carrying a type code and a Greenery ruling block the tool does not have — the selector offers nothing until then; (5) `/properties/:id/sms-number` creates the line outbound-disabled and no door enables it — invites go link-only or `sms_failed`; (6) the prospect agent's legacy dated inventory keys on `units.occupancy_status`/`bedrooms`, which Deal Setup never writes; (7) pricing resolves nothing without unit types (`unit_type_not_established`), which the same mapping run supplies, and pricing authority is conferred only through `tools/release/skyline_grant_authority.js` (no HTTP route). Owner decisions: organization, deal-less legacy activation, bed basis of the real workbook, lines, lease terms and signer, unit types and pricing, card wording, Render allowlists. Not run: production reads, the real workbook, browser, provider. Not registered in `verify_all.sh` (needs a boot variant that allowlists a proof-chosen property id). | `tests/proofs/greenery_launch_rehearsal.db.js`; `docs/handoffs/new-hp/greenery-launch/RECEIPT.md`, `LAUNCH_CHECKLIST.md`, `evidence.rehearsal.json`; `src/identity/super_admin.js` invite; `src/identity/org_admin.js` invite; `src/onboarding/deal_service.js` `addProperty`; `src/tenancy/inventory_materialization.js`; `src/surfaces/availability_read.js` `use_not_configured`; `src/comms/tenant_link.js` sms-number; `src/leasing/leasing_inventory.js` legacy branch |
