...
| # | Defect | Evidence |
|---|---|---|
| 69 | **EVIDENCE ONLY, 2026-09-07 — hold versus inventory closed with actual financial reads on candidate `139e696`.** Same historical mixed fixture plus a real sibling lease (302 Room1, 850) and a governed cycle. Source correction over HTTP changes no financial figure: inventory 5→5, occupancy primitive 1÷5 both times (labels and leases only), leasing desk 20%→20%, canonical rent roll 1 of 5 = 20% both times with the placeholder moving `vacant`→`unresolved` (still in the denominator), trusted contractual rent 850→850, forward rent positions 5 / remaining 4 both times, future facts 5 / 4 both times. Vacancy loss has **no canonical owner** in `src/`; the 8791791 sentence naming it is withdrawn. Down hold: canonical rent roll subtracts the three down rows (1 of 2 = 50%) while the primitive and desk still say 1 of 5 — two occupancy owners disagree under a hold. Retirement: `retired` is a number (1), reason `superseded_by_corrected_inventory_grain`, no date-sensitive reasons; reinstatement leaves 5 spaces, 9 proposals, 9 evidence rows, 2 baselines with supersession links, 2 batches, the artifact sha256, the lease and the cycle byte-identical, and every consumer and financial field equal; the occupancy primitive ignores retirement (no `NOT_RETIRED`). Explainability: for Room1 under down the reads already say basis established + marketing down + bucket open (two axes, readiness untouched); under source correction the hold is expressed by replacing occupancy. Pre-build ruling in the receipt: dispute (no position-grained marketing hold exists; down is unit-grained) versus invalid inventory (retirement owns the meaning, is unit-grained, and only a person's recorded decision establishes it). Rows 66–68 are on the other Fable evidence branches. Rung: `PROVEN` (real Postgres + real HTTP), not in CI by design. | `tests/proofs/hold_vs_inventory_closeout.db.js` (33/33); `tests/proofs/existing_mixed_inventory_hold.db.js` (carried unchanged from 8791791); `docs/handoffs/desktop-qb-20260906/hold-vs-inventory/RECEIPT.md` + scrubbed JSON |
