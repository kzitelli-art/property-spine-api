...
| `from spaces s` correlated walk — `space_position.js` `loadSpaceRows` | **`HTTP_PROVEN` as a MEASUREMENT — examined, NOT bounded, and the reason is the finding** | See `docs/TENANCY_STANDING_COST_RECEIPT.md`. Not deferred for time: the `leases` array is consumed by `position_classifier.js` for an **arbitrary** `asOf` (`datesSpan`, `isFuture`), for the **successor** search, and for the **double-let contest guard**. A count- or recency-bound changes output and can offer an occupied bed as free. **Reclassified, not fixed.** Now observed rather than argued from source — `tests/space_rows_lease_relevance.db.js` 5/5: removing a lease that ENDED before `as_of` changes nothing (so a *date-relevance* predicate is a real candidate, unlike a recency bound); removing the SPANNING lease drives `current_lease_position` → `null`; removing the FUTURE lease changes the forward read. ⚠ I predicted the second would read `none`; it reads `forward` — an occupied bed offered as available *with a story*. Recorded, not smoothed |
