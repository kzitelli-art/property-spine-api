...
**September 12 empty Greenery CI registration:** the existing strengthened Greenery journey now has its own sequential server phase with a fresh synthetic property ID in the canonical owned runner. It must reach the intended empty-home stop; failure fails configured CI. Shell syntax and whitespace checks passed; the new runtime result is pending. This deliberately empty fixture does not rehearse production's 171 legacy unit/space records. No product, migration, provider or production change. See [registration and isolation](handoffs/new-hp/staff-assisted-journey/GREENERY_CI_REGISTRATION.md).
