...
| 8 | **RESOLVED AND DEPLOYED, 2026-08-21.** ~~A signed-in operator's Invite button silently fakes success.~~ Team now requires the live staff session and always calls the API. A signed-in read failure shows `team unavailable / nothing changed`; invite failure creates no local row. Job presets replace editable title/module fields, and the browser never needs the shared operator key for Team. | App `f1ea001`; `team_access_live_boundary.test.js`; full app suite 1,457/1,457 |
