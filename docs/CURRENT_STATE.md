...
**September 12 QB application integration, not released:** integrated governed unsent-draft correction, canonical draft recovery and manual-email preparation through the existing conversion command. The existing capability now composes the canonical exact-home draft read for both desk and conversation; no alternate offer selection or store. Manual preparation and explicit external-email attestation remain separate, text consent remains enforced, and changed explicit expiry conflicts on recovery. Registered draft correction, read recovery and manual-email HTTP proofs in the activated canonical runner; removed the obsolete journey assertion that a pre-invitation correction must fail, preserving the separate post-invitation applicant-review chain. The composed API passes all56 source gates and runner syntax/whitespace checks. Worker evidence: draft HTTP54/read4, manual HTTP20, earlier app browser28; final combined browser and new exact-SHA CI remain pending. Actual Mike's limited pricing authority, current eligible inventory, real website delivery and per-property acceptance are not established by fixture results. Live remains b4a494c/993d408. See [manual preparation](handoffs/new-hp/MANUAL_EMAIL_APPLICATION_20260912.md) and [draft recovery](handoffs/new-hp/draft-offer-correction/READ_RECOVERY.md).
