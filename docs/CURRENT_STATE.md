...
| # | Defect | Evidence |
|---|---|---|
| 68 | **EVIDENCE ONLY, 2026-09-07 — on existing mixed inventory (a confirmed '(whole unit)' sentinel beside confirmed rooms) nothing an operator can do today stops the offer at the position grain without cost.** Exercised over real HTTP on the owned server against a directly-built historical fixture (no import after the guard). Source correction (corrected file, new setup) is the only space-grained path: availability holds the placeholder `occupancy_unknown`, the application selector drops it, the rooms and the neighbouring unit stay marketable, history is superseded not deleted. But the Rent Roll still counts it as a rentable position (`not_established`, denominator unchanged) and standing/Ask says "4 of 5 … 1 do not", never disputed. Marking the unit down and a severe triage both hold the placeholder AND the two real rooms (unit-grained), and neither travels to the Rent Roll bucket or the standing tally (`rentRollBucketOf` reads basis/conflict/evidence only). Retirement is unit-grained and service-only; no space-level retirement is exported. Entitlement refusals: wrong operator key 401 at the down door; foreign-property session 403 at triage; maintenance-only seat 403 at leaseable-units. Ask over HTTP answers `unavailable` under the e2e sentinel with no grounding; facts read through `gatherFacts`/`readTenancyStanding`. Smallest missing connection named for QB (retirement grain at the position, same owner), compared against a broad label rule; neither drafted. Rows 66 and 67 are on the other two Fable evidence branches. Rung: `PROVEN` (real Postgres + real HTTP), not in CI by design. | `tests/proofs/existing_mixed_inventory_hold.db.js` (30/30); `docs/handoffs/desktop-qb-20260906/existing-mixed-inventory/RECEIPT.md` + scrubbed JSON |
