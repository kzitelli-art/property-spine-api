...
| 25 | **SKYLINE STAFF SMS TRANSPORT NOT ACTIVATED, 2026-08-21.** The unified router is deployed, but a real Skyline staff text cannot reach it yet. Staff SMS deliberately enters through one organization-level `operations` line with `staff` audience and `reply_only` outbound policy; Skyline's active property line remains correctly limited to residents and prospects. Production currently has `properties.organization_id = null` for Skyline and no operations line owned by a Skyline organization. The one active operations line belongs to `Demo ORG` and has real historic provider traffic, so it must not be silently rebound under an invented business identity. Next ruling: name the operating organization that owns Skyline; then assign Skyline and explicitly retain/rebind/provision its operations number before Mike's first Ask Spine text. | `src/comms/communication_lines.js`; `src/comms/communications_boundary.js`; `src/comms/tenantlink.js`; production read-only audit 2026-08-21 |
