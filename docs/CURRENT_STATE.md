...
| 68 | **HTTP-PROVEN LOCALLY (owned DB + owned server), 2026-09-12 — establishing a bed-basis opening position on the Greenery legacy shape (171 unit rows, one placeholder each, no use, no lineage, no organization) through the existing owners keeps every legacy identity and leaves the untouched legacy rows in the leasable denominator.** Unmodified tests-only `8cf5078` (product = live `b4a494c`): super-admin adoption → super-admin provisioning of an organization-less existing account (org-admin door refuses 409) → Deal Setup → read-source with bed basis → 106/106 rows confirmed → established. 171 units before and after; 212 spaces (87 beds, 125 placeholders); 84 leases; availability 84 occupied / 22 use_not_configured / 106 occupancy_unknown / 0 marketable; selector empty; classification tool refuses without a Greenery ruling; repeats refused or idempotent. **Findings:** occupancy by basis and the canonical rent roll count the 106 untouched legacy rows as rentable (84 of 212 = 39.6% reported vs 80% over source positions); no product door reconciles, retires or excludes a legacy row; a room-style label in the Unit column resolves to the legacy record, not the parent; the super-admin door sets `users.organization_id` on the attached account. Synthetic labels; the workspace shape file should regenerate the fixture before the numbers are read as Greenery's. 33/0, 16 observations. No product change. | `tests/proofs/greenery_legacy_inventory.db.js`; `tests/e2e/verify_all.sh`; `docs/handoffs/new-hp/greenery-legacy-inventory/RECEIPT.md` |
