...
| 66 | **LOCALLY EXERCISED ON TWO TREES · THREE FINDINGS OPEN · RULINGS OWED, 2026-09-11 — the lead-to-lease journey survives wrong-property sessions, an occupied sibling bed, changed prices and uncertain readiness, and does not survive naming the occupied sibling.** An independent challenge harness walked intake → scripted matching → tour → ready-to-apply capture → offer → Ask Spine proposal → agent confirmation → fake-transport application text → public terms acknowledgement → submission → approval → confirmed terms → packet → resident signing → company execution → tenancy on the exact bed, on owned servers for released `24f4482` (121 passed / 7 failed) and candidate `932c9af` (125 passed / 3 failed). Held on both: foreign-property sessions get 404/403 or empty reads, the leased sibling is never offered, a replacement offer re-hashes the same link and the old hash/form rent/unit `market_rent` cannot move terms, an expected turn date is `expected` not confirmed, start-before-ready and notice-date and task-deadline turns refuse with named codes, a slipped date at submission refuses without birthing an application, and media facts that name a unit stay `exact_home_association: NOT_ESTABLISHED`. Open: (1) both trees — `Send X the application for Unit C7, Bed A` (Bed A leased) proposes **Bed B** (`chooseTarget` falls back to the only offerable target in the hinted unit); (2) candidate — a turn on the outgoing sibling no longer lends its date to the free bed but still marks it `turnover_required`/`incomplete` and drops it from the selector (unit-scoped `turn_status`), where the baseline offered it on the wrong date; (3) baseline only — Matterport/negation/question replies attach a unit historically, clarified on the candidate. Observed, not asserted: offers and applications may be born on an expected date and the applicant context carries no readiness confidence; the tour records unit grain only; the public submit door returns sentences, not stable codes. The earlier `fable_bed_expected_ready.db.js` harness and `RUN_RECEIPT.log` do not exist in either repo's history and are reported unavailable. Not run: browser, provider, production. Not registered in `verify_all.sh` (red by design until ruled). | `tests/proofs/lead_to_lease_challenge.db.js`; `docs/handoffs/new-hp/lead-to-lease-challenge/RECEIPT.md` (+ scrubbed `evidence.baseline.json`, `evidence.candidate.json`); `src/leasing/staff_sms_action.js` `chooseTarget`; `src/tenancy/space_position.js` `turn_status`; baseline reference `tests/e2e/tour_application_lease.e2e.js` 151/151 on `24f4482` |
