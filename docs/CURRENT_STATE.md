...
| 14 | **RESOLVED, 2026-08-20.** ~~`terms[0]` picks the shortest — and dearest — published term when a prospect names none~~. The adapter now agrees with `effective_pricing.js:399`, which had already ruled on this exact case: with no term supplied it refuses `lease_term_not_selected` and returns the published **menu**. **One term is not a choice** — a property publishing a single term has nothing to disambiguate, so it is quoted and the term stated; refusing there would hand off on a question with one answer. With two or more, the refusal carries `published_terms` and a question to the prospect (*"which length are you looking for?"*). The agent needed no change: its not-quotable branch already forbids a figure and hands the model `say` verbatim. **Proof rung: real-DB, `tests/e2e/agent_pricing_wall.e2e.js` 22/22, in CI.** Proving it required a SECOND property — a published version's terms are frozen, so a term cannot be added to one already published; the trigger refuses the INSERT, correctly. Asserted: no number returned, reason matches `effective_pricing`'s own, the dearest term withheld, menu `[5,12]`, prospect asked to choose, and naming either term returns that term's own rent. Not deployed — code is on the branch, production still runs `bcd3089`. | `tests/e2e/agent_pricing_wall.e2e.js` §5 |
...
| **Pending tenancy creation** | **EXISTS AND IS WIRED** — this row was wrong | **Corrected 2026-08-22** by the doc-reconciliation pass, verified independently here. Two writers were conflated. `tenancy_anchor_service.js:284` — confirm-term — inserts `lease_status='pending'`; route `operator.js:4199`, wired `server.js:3260`/`:3500`. `activation_service.js:700` is a DIFFERENT writer, the rent-roll ledger importer, where `'active'` is correct because those tenancies already exist. **What was observed correctly is narrower: the PHRASE "pending tenancy" appears nowhere.** A vocabulary absence was read as a capability absence — the exact failure this register exists to prevent, occurring inside it, and telling every new thread to build something that already works. Rung unchanged pending a run; this corrects the existence claim, not the proof level. | `src/tenancy/tenancy_anchor_service.js:284` · `src/identity/operator.js:4199` |
