...
| # | Defect | Evidence |
|---|---|---|
| 66 | **CANDIDATE, 2026-09-07 — an accepted occupied opening claim with no operative lease no longer falls through to `marketable_now`.** `availability_read.marketingState` consumed neither `evidence_state: uncorroborated` nor `basis_type: opening_claim_occupied`, which the dated position already supplies, so a bed the unit Rent Roll bucketed `occupied` and standing counted occupied was offered `marketable_now`, listed by `leaseable-units` and accepted by the application authority (witness 18/18 on `3df9ed9`, over HTTP). The candidate returns the existing `occupied` state with reason `opening_claim_occupied_uncorroborated` and the same blocking fact, placed below the lease, commitment, possession, turnover, unknown-basis and unreconciled guards and above the triage overlay; uncorroborated stays distinct from `evidence_disagrees`. Successor 18/18; unit regression red on the parent, 5/5 on the candidate; space-availability, readiness-axis, relay-edges, unattached and identity proofs unchanged. Preserved: confirmed vacancy offered, native lease `spanning_lease`, pending future commitment, contested, down precedence, readiness overlay on vacancies, unknown basis, maintenance-only 403. The canonical rent roll keeps `unresolved` on its contractual axis by design. Section 0 shows the current confirmation writer creates a lease for an occupied row, so the shape is retained data. No app change (no new state). Browser not run. Numbered 66 on this branch because the defect gate requires contiguous numbering; the evidence rows 66–70 on the earlier Fable evidence branches will need renumbering when any of them is integrated. The proof runs inside the owned-server block of `verify_all.sh` (after the server is UP, beside the other HTTP proofs); the proof snapshots every lease row before reads and compares after. Rung: `PROVEN` (real Postgres + real HTTP), in `verify_all.sh`. | `src/surfaces/availability_read.js`; `tests/unit/availability_occupancy_basis.test.js`; `tests/proofs/availability_uncorroborated_claim.db.js` (witness/successor); `tests/e2e/verify_all.sh`; `docs/handoffs/desktop-qb-20260906/uncorroborated-claim/RECEIPT.md` + two scrubbed JSON artifacts |
