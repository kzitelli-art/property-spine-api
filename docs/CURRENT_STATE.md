...
| # | Defect | Evidence |
|---|---|---|
| 70 | **EVIDENCE ONLY, 2026-09-07 — occupancy has three visible percentages with two unlabelled sets, and the canonical ratio breaks under a hold.** Fixture with occupied, vacant, unresolved (source-occupied, no lease) and contested positions on API `edceeb5`, app read at `8d1da84`. Baseline: canonical/institutional 2 of 7 = 28.57% (occupied ÷ inventory − down − contested; unresolved stays in the denominator); primitive/condition/desks 3 of 8 = 37.5% (leases ÷ spaces, label exclusions); legacy snapshot 4 of 8 = 50% (source status words, what the Management home "Current occupancy" cell is computed from in the browser); standing/Ask counts only; availability states only. **Defect A:** the canonical numerator counts `is_down` rows the denominator excludes: a hold on a unit with an operative lease reads 2 of 5 = 40%, holding every occupied and contested unit reads 2 of 1 = 200%, all-down reads 2 of −1 = −200%, printed on the institutional screen and CSV as "Confirmed contractual occupancy: 2 of −1"; the null guard is never reached. **Defect B:** 303 Room2 is `unresolved` (canonical), `occupied` (unit Rent Roll bucket, standing/Ask) and `marketable_now` (availability, application selector) at once. Holds never changed a lease row or a tenancy_state; every owner returned to baseline after resolve. The authenticated Leasing home does not paint the condition strip (owner correction), so the condition API has no visible signed-in consumer. Smallest owner corrections proposed (one population for the canonical ratio; availability reading the Rent Roll's tenancy_state for `unresolved`; label copy naming the set); none implemented. Closeout proof's tautological bucket assertion and resolve wording corrected on this branch. Rows 66–69 are on the other Fable evidence branches. Rung: `PROVEN` (real Postgres + real HTTP); screen labels source-read, no browser; not in CI by design. | `tests/proofs/occupancy_measures_under_holds.db.js` (30/30); `tests/proofs/hold_vs_inventory_closeout.db.js` (34/34, corrected); `docs/handoffs/desktop-qb-20260906/occupancy-meaning/RECEIPT.md` + two scrubbed JSON artifacts |
