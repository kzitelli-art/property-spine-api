...
**September 12 current combined candidate, not released:** the existing conversation reply route now records an explicitly attested external email through the interactions owner, with server-scoped actor/recipient, immutable key/body/time replay, retained occurrence/capture provenance and no provider send. Queue, Person and Ask read the same event; waiting-on can become prospect while human takeover custody remains in progress until explicit handback. Later inbound returns attention to the same owner. Owned HTTP36, unchanged inquiry-state42 and56 source gates passed before integration; the canonical runner now requires that proof. Staff composer/browser remains active, and the broader app harness is still under test-only repair. The real-label Greenery rehearsal215c334 passed exact CI34695251765/run47640/0; QB integrated it and strengthened original unit/space-ID retention and exact export-convention assertions, pending combined CI. Its84 occupied/21 vacant and80% are synthetic fixture values. Live remains b4a494c/993d408; no production establishment or new deployment. See [external reply receipt](handoffs/new-hp/website-reply/EXTERNAL_EMAIL_REPLY.md) and [Greenery review](handoffs/new-hp/greenery-legacy-inventory/QB_REVIEW.md).
