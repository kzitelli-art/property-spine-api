...
| # | Defect | Evidence |
|---|---|---|
| 67 | **EVIDENCE ONLY, 2026-09-07 — the current onboarding writer creates '(whole unit)' beside a real bed, and offers both.** Through real HTTP on the owned server (upload → activation → read-source → confirm ×2 → establish → availability-canonical / leaseable-units / rent-roll/units), two current rows naming `(whole unit)` and `Room1` on one fresh by-bed unit become two established positions, both governed `position_kind = 'bed'`, both `marketable_now`, both application-offerable, in either row order, with no refusal and no discrepancy note. Cause: `labelsByUnit` carries the sentinel, materialization keeps the placeholder when wanted includes it and stamps `kind` on every wanted label; the count invariant holds by construction. A hold keyed on mixed `position_kind` would not fire (QB's objection confirmed). Controls green: by-unit single position; sole bed `Bed B`; the e2e fixture shape re-read (unclaimed placeholder held `occupancy_unknown`); three beds; a valid bed property with a whole-unit unit beside bed units (that placeholder's governed kind is null, never materialized); a leased whole-unit position refuses conversion (`PLACEHOLDER_NOT_PRISTINE`, over HTTP as `refused` 409); future-only rooms create nothing. Correction path today: materialization refuses (`PLACEHOLDER_NOT_PRISTINE` held by `import_source_rows.produced_space_id`); same file name + as-of is refused as already read; a renamed corrected file in a new setup supersedes the baseline and demotes the phantom to `occupancy_unknown` but removes nothing; no position-level removal writer exists in `src/`. Proposed, not built: refuse in `materializeRentableSpaces` when wanted includes the sentinel and has more than one label (`MIXED_GRAIN_LABELS`), keyed on Spine's own sentinel, not on a source label. Row 66 is on `claude/identity-readiness-evidence-20260907`. Rung: `PROVEN` (real Postgres + real HTTP), not in CI by design. | `tests/proofs/mixed_grain_writer_challenge.db.js` (43/43); `docs/handoffs/desktop-qb-20260906/mixed-grain-writer/RECEIPT.md` + scrubbed JSON |
