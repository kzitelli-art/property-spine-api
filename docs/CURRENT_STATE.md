...
| 14 | **`quotablePricing()` picks a lease term by array order, not by rule, when a prospect doesn't name one.** `pricing_adapter.js`, `terms.find(...) : terms[0]` — with multiple published terms (e.g. 6-month and 12-month), the "no term specified" case silently takes whichever is first in the array. **Distinct from defect #1, not the same severity**: the term IS disclosed downstream (`agent.js:515` renders *"rent $X/mo on a Y-month term (governed published pricing)"*), so a prospect isn't quoted a bare number with no context — but which term gets offered first is arbitrary rather than deliberate (shortest? most commonly requested? explicitly ask the prospect?). Found independently, not yet assessed for why it's on the reporting thread's critical path — their message describing the concrete failure case was cut off mid-sentence before it reached this file. | `src/agent/pricing_adapter.js` ~line 101 |
