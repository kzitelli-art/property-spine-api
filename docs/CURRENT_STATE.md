...
> ⛔ **THE BLOCK ABOVE AND THE PARAGRAPH BELOW ARE BOTH STALE AS OF 2026-08-24.**
> `RESOLVED #7` and `matches production` were true on 20 Aug and false by 22 Aug.
> **See defect #28.** Current truth: `main` = `41dea52`, ceiling **187**;
> production = `61f99bf`, ledger **189**; `61f99bf` is **not** an ancestor of
> `main`. `main` is fully audited and **cannot boot**. Left in place rather than
> rewritten, deliberately — the stale claim and its correction sitting together
> is the honest record of how this file failed and how it caught itself.

✔ ~~**RESOLVED 2026-08-20: production now runs `main`'s lineage.**~~ *(superseded — see #28)* Was: the
...
| 28 | **⛔ RECURRENCE OF #7, NOT A NEW DEFECT — production does not run `main`, again, within 48 hours of #7 being marked resolved.** Verified 2026-08-24: `origin/main` = `41dea52`, migration ceiling **187**. Production = `61f99bf`, ledger **189**, and `git merge-base --is-ancestor 61f99bf origin/main` returns **NO**. **`main` is clean, fully audited, and cannot boot** — 187 migration files against a 189 database. Two further branches carry migrations 190 and 191 (`9746f12` Codex Skyline lease; `3173913` another CC thread, +62 vs main), so the gap widens, not closes. **Reconciliation is deliberately held** — the owner is on a laptop for two weeks and cannot safely release 190/191 against a 189 ledger from there. ⚠ **Read #7 and this row together.** #7 records the resolution; this records that the resolution did not hold. This document's own "reunited" claim was true when written on 2026-08-20 and false by 2026-08-22 — recorded as the register catching its own author, which is the failure mode it exists to catch. Do not mark #7 resolved again without checking `merge-base` on the day of the claim. | `git merge-base --is-ancestor 61f99bf origin/main` → **NO**; `main` ceiling 187 vs ledger 189 |
