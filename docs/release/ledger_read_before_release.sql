-- Generated from the ACTUAL migration files on main @ 7ebb400.
-- REGENERATED 2026-08-12. The previous version's build list ended at 159 and
-- had no knowledge of 160-167, so it would have reported a clean `pending`
-- for a set it never looked at -- the migration 140 failure mode described in
-- the footer, reproduced by the file that warns about it.
...
--
--  AS OF THIS REGENERATION, 162-167 ARE RELEASED and the expected ceiling
--  is 167 — so `pending` should come back EMPTY until a new migration
--  lands. An empty pending is the correct answer here, not a broken query.
--  Read the ceiling anyway. Never type it from this comment.
--
