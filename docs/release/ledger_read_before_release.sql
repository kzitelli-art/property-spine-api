-- Generated from the ACTUAL migration files on main @ c7f4dc4.
-- REGENERATED 2026-08-15. The previous version's build list ended at 167 and
-- had no knowledge of 168-171. Against a ledger already at 170 it would have
-- reported those versions under `in_ledger_but_not_in_build` -- which this
-- file's own footer defines as "the code and the schema disagree" -- and it
-- would have said so on the eve of the 171 release. Same root cause as the
-- 2026-08-12 regeneration and as migration 140: a build list that describes
-- fewer files than the repository actually contains.
-- Not a hand-typed range: that is exactly how migration 140 was missed.
-- The 125 and 138/139 gaps are real and are preserved.
...
--
--  AT THIS REGENERATION the build carries 159 versions, 001 through 171.
--  The last reported production ceiling was 170 (Codex build summary,
--  2026-08-15) and 171 merged the same day, so `pending` is EXPECTED to
--  return exactly 171 and nothing else.
--
--  That expectation is REPORTED, not verified. It is written here so a
--  surprise is visible, never so the ceiling can be typed from it. Read
--  the ceiling from the query every time.
--
