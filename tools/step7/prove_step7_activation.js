...
(async function main() {
  ...
  /*  THE PRECONDITION IS 140, NOT A CEILING.
   *
   *  This demanded ceiling exactly "137" and then, four lines later, read
   *  release_0_activation_history — a table migration 140 creates. Both
   *  cannot be true: at ceiling 137 that table does not exist and the
   *  query below throws. The check was wrong in a way that only showed up
   *  once a baseline was built that could actually satisfy the tool.
   *
   *  What it needs is the guard schema from 140. Anything above it is a
   *  database closer to production, not further from it — main's ceiling
   *  is 152 and that is the shape this proof should run against. */
  const pre = await c.query(
    `select max(version) ceiling,
            count(*) filter (where version = '140') has140
       from schema_migrations`);
  const ledger = pre.rows[0].ceiling;
  if (Number(pre.rows[0].has140) !== 1) {
    console.error(`REFUSED: migration 140 is not applied (ledger ceiling ${ledger}). ` +
      "This proof exercises the activation transaction against the guard 140 installs; " +
      "without it there is nothing to activate and every assertion would be vacuous.");
    process.exit(2);
  }
  console.log(`  baseline ledger ceiling ${ledger}, migration 140 applied.`);
