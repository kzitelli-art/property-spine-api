...
(async function main() {
  ...
  /*  F2b LIVES HERE NOW. It asks whether the deployed READER shares the
   *  deployed WRITER's evidence vocabulary — a source-alignment question
   *  about a later boundary, never a fact about this completion. It is
   *  printed with the surface section and carried as its own conclusion. */
  const align = F.readerSourceAlignment();
  console.log("");
  console.log("  " + (align.status === "PASS" ? "ok  " : "·   ") +
    "A1  reader/writer evidence vocabulary — " + align.status);
  console.log("          writer (deployed, decided F2): " + JSON.stringify(align.writer_array));
  console.log("          reader (deployed):             " + JSON.stringify(align.reader_array));
  console.log("          " + align.why.replace(/(.{66}) /g, "$1\n          "));
  if (align.becomes_required_at) {
    console.log("          REQUIRED AT: " + align.becomes_required_at +
      " — the surface clause cannot discharge until it is PASS.");
  }
  ...
  const receipt = {
    ...
    /*  TWO CONCLUSIONS, DELIBERATELY SEPARATE.
     *
     *  `completion_writer_proof` is the Boundary 5 verdict and rests
     *  ONLY on persisted production facts — B, F and N. It is what the
     *  exit code follows.
     *
     *  `reader_source_alignment` is a claim about two source files, one
     *  of which belongs to Boundary 9. It may be DEFERRED here and must
     *  be PASS before the surface clause discharges. It never reddens
     *  the verdict above; that coupling is the defect this corrects. */
    step4_package_revision: F.PACKAGE_REVISION,
    step4_package_revision_is: F.PACKAGE_REVISION_IS,
    completion_writer_proof: "PASS",
    completion_writer_proof_rests_on: "persisted production facts only (B, F, N)",
    evidence_vocabulary_source: F.CLASSIFICATION_SOURCE,
    reader_source_alignment: align.status,
    reader_source_alignment_detail: align,
