...
//  §3.1 corrected array. `unclassified` is NOT proof of a repair. Stated
//  here rather than imported from the reader so that a change to the
//  reader cannot silently redefine what this proof accepted — the two
//  agreeing is itself asserted, by F2b.
const PROOF_CLASSIFICATIONS = ["repair_photo", "condition"];
...
async function checkCompletionFacts(db, {
  ...
  //  The array this proof accepted must be the array the READER enforces.
  //  Two copies that drift is exactly the §3.1 defect, one level up.
  let readerArray = null;
  try {
    // eslint-disable-next-line global-require
    readerArray = require("../../src/surfaces/work_order_status_read.js").PROOF_REQUIRED_CLASSIFICATIONS;
  } catch (_) { readerArray = null; }
  facts.push(fact("F2b", "this proof's classification array matches the reader's",
    Array.isArray(readerArray) &&
      JSON.stringify([...readerArray].sort()) === JSON.stringify([...PROOF_CLASSIFICATIONS].sort()),
    "the proof and the reader disagree about what counts as proof",
    { proof_array: PROOF_CLASSIFICATIONS, reader_array: readerArray }));
...
module.exports = {
  PROOF_CLASSIFICATIONS, VERIFIED_MIME,
