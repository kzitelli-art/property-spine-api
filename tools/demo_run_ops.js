// ════════════════════════════════════════════════════════════════════
//  CLASS 3 — DEMO-RUN OPERATIONS, OUTSIDE THE DEPLOYED HTTP RUNTIME.
//
//  A verbatim copy of src/leasing/demo.js as it stood at 0b70fd9, retained so
//  the boardroom-demo bootstrap logic is not lost. The four MUTATING routes
//  were removed from the server in the same commit; the read-only
//  GET /demo/runs/:slug/state stays mounted there. This file is the record of
//  what the removed routes did, and the basis for a controlled operations path
//  if one is ever needed.
//
//  WHY THEY WERE REMOVED (source-proven; the routes were never invoked):
//  demo.js has ZERO process.env references — no DEMO_MODE, no access code, no
//  confirm token — and mounts under /demo/, which server.js lists in
//  PUBLIC_PREFIXES. Every route below was reachable UNAUTHENTICATED:
//    POST /demo/runs/:slug/seed — accepts a CLIENT-SUPPLIED property_id,
//      validates only that the property exists, then writes
//      property_team_assignments with can_manage_roles=true on it, and creates
//      properties / units / users rows. §21 inverted: a client-provided
//      property ID deciding where authority is granted.
//    POST /demo/runs/:slug/reset — accepts client-supplied manager_user_id and
//      tenant identity; mints durable `persons` rows.
//    POST /demo/runs/:slug/application-submit — overwrites a real `persons`
//      row's name/email/phone from an unauthenticated body; creates a real
//      lease_application through the canonical submission service.
//    POST /demo/runs/:slug/application-approve — invokes the canonical
//      approveApplication attributed to a real manager user the caller never
//      authenticated as, closing the approval gate and creating the
//      terms_review birth obligation.
//
//  ⚠️ THIS FILE IS NOT WIRED. It still exports an express router, so it must
//  NEVER be mounted. Nothing in src/ or server.js may require it. Reinstating
//  any capability means building an authenticated, operator-scoped path — not
//  re-mounting this module.
//
//  REMOVAL CONDITION: delete once the boardroom demo runs entirely through
//  canonical authenticated operator paths, or once its operating procedure has
//  a controlled operations home.
//
//  BUILT ONLY — not executed against any database in the slice that created it.
// ════════════════════════════════════════════════════════════════════
