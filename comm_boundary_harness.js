...
(async () => {
  ...
  await pool.query(`insert into leases (property_id, tenant_ids, lease_status) values ($1, array[$2::uuid], 'active')`, [propA.id, resA.id]);
  await pool.query(`insert into leases (property_id, tenant_ids, lease_status) values ($1, array[$2::uuid], 'active')`, [propB.id, resB.id]);
  await pool.query(`insert into tenant_invites (person_id, property_id, status) values ($1,$2,'used'),($3,$4,'used')`, [resA.id, propA.id, resB.id, propB.id]);
  ...
  }
  {
    ...
    await pool.query(`insert into leases (property_id, tenant_ids, lease_status) values ($1, array[$2::uuid], 'active'),($1, array[$3::uuid], 'active')`, [propA.id, dup1.id, dup2.id]);
    await pool.query(`insert into tenant_invites (person_id, property_id, status) values ($1,$2,'used'),($3,$2,'used')`, [dup1.id, propA.id, dup2.id]);
