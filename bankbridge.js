...
module.exports = function bankBridgeModule({ pool, spawnObligationFromEvent, satisfyObligation, completeObligation }) {
  ...
  // ──────────────────────────────────────────────────────────────────
  async function bridgeTxn(client, { txn, vendorName, property_id, confirmed_category, confirmed_by_person_id, what_for, mode }) {
  ...
  router.post("/bank/transactions/:id/unbridge", async (req, res) => {
    ...
    try {
      ...
      const prov = me.rows[0] ? me.rows[0].provenance : null;
      const provSource = prov && typeof prov === "object" ? prov.source : (prov ? JSON.parse(prov).source : null);
      ...
      const oblId = me.rows[0].obligation_id;
      if (oblId) {
        await client.query("update money_events set obligation_id = null where id = $1", [meId]);
        // delete known obligation child rows first (same guard as money.js test-cleanup)
        const existing = await client.query(
          `select table_name from information_schema.tables
            where table_schema = 'public'
              and table_name = any($1)`,
          [["obligation_inputs", "obligation_proofs", "obligation_events"]]);
        for (const row of existing.rows) {
          await client.query(`delete from ${row.table_name} where obligation_id = $1`, [oblId]);
        }
        await client.query("delete from obligations where id = $1", [oblId]);
      }
      await client.query("delete from money_events where id = $1", [meId]);
      ...
      await client.query("commit");
      return res.json({ unbridged: txnId, deleted_money_event: meId, deleted_obligation: oblId || null,
