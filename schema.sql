-- ════════════════════════════════════════════════════════════════════
--  PROPERTY SPINE — schema v1
--  Design principle: the SCHEMA expresses the full thesis (control system).
--  The built ENDPOINTS stay narrow (today: create/read property).
--  Tables exist now so later layers attach without a rebuild.
--
--  The loop everything serves:
--    Event → Obligation → Required Input → Clock → Escalation → Proof → Record
-- ════════════════════════════════════════════════════════════════════

-- Enable UUID generation (pgcrypto ships with Postgres / Neon).
