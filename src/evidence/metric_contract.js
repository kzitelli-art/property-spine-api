...
const FUNNEL_CONTRACTS = Object.freeze({
  ...
  f2: Object.freeze({
    ...
    deduplication_key: "leasing_conversions.id",
    source_attribution_basis: SOURCE_ATTRIBUTION_BASIS,
    //  Self-contained, deliberately: a frozen contract is read standalone, so
    //  no field may require dereferencing another funnel's contract.
    pending_definition: "a real unresolved act from the lifecycle authority (future appointment, visit awaiting outcome, open obligation, application underway) — never 'not terminal'",
    terminal_definition: "ordered lifecycle events with exact conversion_id (migration 128); mutable status labels are never evidence",
    conflict_conditions: "competing or contradictory appointment attribution; a chain whose members carry different opportunities; duplicate terminal events at one sequence",
    untrackable_conditions: "reschedule cycles; parents outside the opportunity; historical events without exact attribution; unlinked applications that MAY belong make the row ambiguous and the cohort partial",
