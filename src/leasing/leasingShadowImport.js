"use strict";
// ════════════════════════════════════════════════════════════════════════
//  leasingShadowImport.js — the Skyline RIDE-ALONG resolver (Layer 1 + 2)
//
...
//  smsForEvent. (Live intake with outreach stays in leasingleads.js.)
//
...
module.exports = function leasingShadowImportModule({ pool }) {
  ...
  // ── phone normalization: identical rule to leasingscheduling.js /
  //    leasingleads.js. null return == structurally invalid/unusable phone. ──
  ...
  async function tx(fn, res) {
    ...
    try {
      ...
      console.error("leasingShadowImport:", e.message);
