...
//    const dealIntakeModule = require("./deal_intake");
...
module.exports = function dealIntake(deps) {
  ...
  if (!pool) throw new Error("deal_intake requires a pool");
  if (!anthropic) throw new Error("deal_intake requires the anthropic client");
  if (!registryInstance || !registryInstance.resolveOnly) throw new Error("deal_intake requires registryInstance with resolveOnly");
  if (!fileToText) throw new Error("deal_intake requires fileToText");
  if (!runIngestAuto) throw new Error("deal_intake requires runIngestAuto");
  if (!upload) throw new Error("deal_intake requires the multer upload instance");
