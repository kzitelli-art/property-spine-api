...
(async () => {
  ...
  check(`forged inbound webhook refused with 4xx (got ${r1.status}; any refusal + zero writes is the invariant)`,
  ...
  check(`provider-event injection refused with 4xx (got ${r2.status})`, r2.status >= 400 && r2.status < 500);
  ...
  check(`provider-event forged status callback refused with 4xx (got ${r2b.status})`, r2b.status >= 400 && r2b.status < 500);
  ...
  check(`intake without secret refused with 4xx/503 (got ${r3.status})`,
    (r3.status >= 400 && r3.status < 500) || r3.status === 503);
  ...
  if (INTAKE_SECRET) {
    ...
    check(`intake to a NON-entitled property refused (got ${r3b.status})`,
      (r3b.status >= 400 && r3b.status < 500) || r3b.status === 503);
  } else {
